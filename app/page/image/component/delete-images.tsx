...
export default function DeleteImages() {
  ...
  const handleDelete = async () => {
    ...
    if (currentTab === "public") {
      const confirmDelete = confirm("Are you sure you want to delete ALL public images?");
      if (!confirmDelete) return;

      const res = await mutation<{ message: string }>({
        url: `${process.env.NEXT_PUBLIC_BACKEND_URL}/images/delete-public`,
        method: "DELETE",
        headers: {
      ...
      const confirmDelete = confirm("Are you sure you want to delete ALL your private images?");
      if (!confirmDelete) return;

      const res = await mutation<{ message: string }>({
        url: `${process.env.NEXT_PUBLIC_BACKEND_URL}/images/delete-private`,
        method: "DELETE",
        headers: {
          Authorization: getCookie("token=") || "",
        },
